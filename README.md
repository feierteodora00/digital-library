...
```bash
git clone https://github.com/feierteodora00/digital-library.git
```
...
There is no configuration required for the database as all endpoints are connected to a MySQL server via phpMyAdmin (provided by university).

Credentials to access functionalities for different user types: 
`teo@example.com teopassword` - admin
`jake@example.com jakepassword` - parent
`ada@example.com adapassword` - teacher

## In order for the chat to function

Navigate under /src/components/server
